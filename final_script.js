...
// Static middleware only for non-API paths to prevent it intercepting PUT/DELETE on /api routes
app.use((req, res, next) => { if (req.path.startsWith('/api')) return next(); express.static(path.join(__dirname))(req, res, next); });
...
async function setupDB() {
  try {
    ...
    await pool.request().query(`IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id=OBJECT_ID('SalaryComponents') AND name='UpdatedOn') ALTER TABLE SalaryComponents ADD UpdatedOn DATETIME DEFAULT GETDATE()`);
    ...
    // Drop legacy AttDate column if it exists (old schema used AttDate instead of CheckIn)
    // Drop ALL objects referencing AttDate: indexes, default constraints, check constraints
    await pool.request().query(`
      IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id=OBJECT_ID('Attendance') AND name='AttDate')
      BEGIN
        DECLARE @attSql NVARCHAR(MAX) = '';
        -- Drop UNIQUE constraints (must use DROP CONSTRAINT, not DROP INDEX)
        SELECT @attSql = @attSql + 'ALTER TABLE Attendance DROP CONSTRAINT [' + i.name + ']; '
        FROM sys.indexes i
        JOIN sys.index_columns ic ON i.object_id = ic.object_id AND i.index_id = ic.index_id
        JOIN sys.columns c ON ic.object_id = c.object_id AND ic.column_id = c.column_id
        WHERE i.object_id = OBJECT_ID('Attendance') AND c.name = 'AttDate' AND i.is_unique_constraint = 1;
        -- Drop plain indexes (not constraint-backed)
        SELECT @attSql = @attSql + 'DROP INDEX [' + i.name + '] ON [Attendance]; '
        FROM sys.indexes i
        JOIN sys.index_columns ic ON i.object_id = ic.object_id AND i.index_id = ic.index_id
        JOIN sys.columns c ON ic.object_id = c.object_id AND ic.column_id = c.column_id
        WHERE i.object_id = OBJECT_ID('Attendance') AND c.name = 'AttDate'
          AND i.is_primary_key = 0 AND i.is_unique_constraint = 0 AND i.type > 0;
        SELECT @attSql = @attSql + 'ALTER TABLE Attendance DROP CONSTRAINT [' + dc.name + ']; '
        FROM sys.default_constraints dc
        JOIN sys.columns c ON dc.parent_object_id = c.object_id AND dc.parent_column_id = c.column_id
        WHERE c.object_id = OBJECT_ID('Attendance') AND c.name = 'AttDate';
        SELECT @attSql = @attSql + 'ALTER TABLE Attendance DROP CONSTRAINT [' + cc.name + ']; '
        FROM sys.check_constraints cc
        JOIN sys.columns c ON cc.parent_object_id = c.object_id AND cc.parent_column_id = c.column_id
        WHERE c.object_id = OBJECT_ID('Attendance') AND c.name = 'AttDate';
        IF @attSql <> '' EXEC(@attSql);
        ALTER TABLE Attendance DROP COLUMN AttDate;
      END
    `);
    ...
    // NOTE: The migration body is wrapped in EXEC() so SQL Server only parses/validates
    // column names at runtime (inside the IF block), not at compile time.
    // Without EXEC(), SQL Server validates 'MonthInt' as a column even when the
    // IF condition is false, causing "Invalid column name 'MonthInt'".
    ...
    const monthMigrationSql =
      "IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id=OBJECT_ID('Payroll') AND name='Month' AND system_type_id=TYPE_ID('nvarchar')) " +
      "EXEC(" +
        "'ALTER TABLE Payroll ADD MonthInt INT NULL; " +
        "UPDATE Payroll SET MonthInt=CASE Month " +
          "WHEN ''January'' THEN 1 WHEN ''February'' THEN 2 WHEN ''March'' THEN 3 " +
          "WHEN ''April'' THEN 4 WHEN ''May'' THEN 5 WHEN ''June'' THEN 6 " +
          "WHEN ''July'' THEN 7 WHEN ''August'' THEN 8 WHEN ''September'' THEN 9 " +
          "WHEN ''October'' THEN 10 WHEN ''November'' THEN 11 WHEN ''December'' THEN 12 " +
          "ELSE TRY_CAST(Month AS INT) END; " +
        "IF EXISTS(SELECT 1 FROM sys.key_constraints WHERE name=''UQ_Payroll'') " +
          "ALTER TABLE Payroll DROP CONSTRAINT UQ_Payroll; " +
        "ALTER TABLE Payroll DROP COLUMN Month; " +
        "EXEC sp_rename ''Payroll.MonthInt'',''Month'',''COLUMN''; " +
        "ALTER TABLE Payroll ALTER COLUMN Month INT NOT NULL;'" +
      ")";
    await pool.request().query(monthMigrationSql);
    ...
    await pool.request().query(`IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id=OBJECT_ID('BonusRequests') AND name='AdminNote') ALTER TABLE BonusRequests ADD AdminNote NVARCHAR(200)`);
    await pool.request().query(`IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id=OBJECT_ID('BonusRequests') AND name='ReviewedOn') ALTER TABLE BonusRequests ADD ReviewedOn DATETIME`);
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    const er = await q(`SELECT ec.CredID,ec.EmpID,ec.MustChange,ec.LastLogin,e.Name,d.DeptName,r.RoleName FROM EmployeeCredentials ec JOIN Employees e ON ec.EmpID=e.EmpID JOIN Departments d ON e.DeptID=d.DeptID JOIN Roles r ON e.RoleID=r.RoleID WHERE ec.EmpID=@u AND ec.Password=@p AND e.Status='Active'`, { u:u.toUpperCase(), p });
...
app.post('/api/employees', requireAdmin, async (req, res) => {
  ...
  if (Phone && Phone.replace(/\D/g,'').length > 10) return res.status(400).json({ error:'Phone number cannot exceed 10 digits' });
  try {
    const exists = await q(`SELECT 1 AS cnt FROM Employees WHERE EmpID=@id`, { id:EmpID });
    if (exists.recordset.length) {
      const last = await q(`SELECT TOP 1 EmpID FROM Employees ORDER BY EmpID DESC`);
      const lastNum = parseInt((last.recordset[0]?.EmpID||'EMP000').replace(/\D/g,''))||0;
      const suggested = 'EMP' + String(lastNum+1).padStart(3,'0');
      return res.status(400).json({ error:`Employee ID ${EmpID} already exists. Try: ${suggested}` });
    }
    await q(`INSERT INTO Employees(EmpID,Name,DeptID,RoleID,BasicSalary,JoiningDate,Email,Phone) VALUES(@id,@n,@di,@ri,@s,@j,@em,@ph)`, { id:EmpID,n:Name,di:parseInt(DeptID),ri:parseInt(RoleID),s:parseInt(BasicSalary),j:JoiningDate,em:Email||null,ph:Phone||null });
    await q(`INSERT INTO SalaryComponents(EmpID,BasicSalary) VALUES(@id,@s)`, { id:EmpID,s:parseInt(BasicSalary) });
  ...
  } catch(err) { console.error('[POST /employees]', err.message); res.status(500).json({ error:err.message }); }
});
app.put('/api/employees/:id', requireAdmin, async (req, res) => {
  const { Name,DeptID,RoleID,BasicSalary,JoiningDate,Email,Phone,Status } = req.body;
  if (!Name||!DeptID||!RoleID||!BasicSalary||!JoiningDate) return res.status(400).json({ error:'Missing required fields' });
  if (Phone && Phone.replace(/\D/g,'').length > 10) return res.status(400).json({ error:'Phone number cannot exceed 10 digits' });
  try {
    const result = await q(`UPDATE Employees SET Name=@n,DeptID=@di,RoleID=@ri,BasicSalary=@s,JoiningDate=@j,Email=@em,Phone=@ph,Status=@st WHERE EmpID=@id`, { n:Name,di:parseInt(DeptID),ri:parseInt(RoleID),s:parseInt(BasicSalary),j:JoiningDate,em:Email||null,ph:Phone||null,st:Status||'Active',id:req.params.id });
    await q(`UPDATE SalaryComponents SET BasicSalary=@s WHERE EmpID=@id`, { s:parseInt(BasicSalary),id:req.params.id });
    await audit(actor(req),'Edit Employee',req.params.id,`Updated ${Name}`);
    res.json({ ok:true });
  } catch(err) { console.error('[PUT /employees]', err.message); res.status(500).json({ error:err.message }); }
});
...
app.post('/api/attendance/checkin', requireAuth, async (req, res) => {
  ...
  try {
    ...
    let status = (h<9||(h===9&&m<=30))?'Present':(h<10||(h===10&&m<=30))?'Late':(h<13)?'Half-day':'Absent';
    if (ex.recordset.length) {
...
app.get('/api/attendance/today-detail', requireAdmin, async (req, res) => {
  const today = new Date().toISOString().slice(0,10);
  const status = req.query.status;
  try {
    if (status === 'onleave') {
      const r = await q(`SELECT e.EmpID,e.Name,d.DeptName,l.LeaveType,l.Reason,l.FromDate,l.ToDate FROM Leaves l JOIN Employees e ON l.EmpID=e.EmpID JOIN Departments d ON e.DeptID=d.DeptID WHERE l.Status='Approved' AND CAST(l.FromDate AS DATE)<=@d AND CAST(l.ToDate AS DATE)>=@d`, { d:today });
      return res.json(r.recordset);
    }
    let where = status ? `AND a.Status=@st` : '';
    const params = { d: today };
    if (status) params.st = status;
    const r = await q(`SELECT a.EmpID,e.Name,d.DeptName,a.Status,a.CheckIn,a.CheckOut,a.OvertimeHours FROM Attendance a JOIN Employees e ON a.EmpID=e.EmpID JOIN Departments d ON e.DeptID=d.DeptID WHERE CAST(a.CheckIn AS DATE)=@d ${where} ORDER BY e.Name`, params);
    res.json(r.recordset);
  } catch(err) { res.status(500).json({ error:err.message }); }
});
...
app.get('/api/dashboard/admin', requireAdmin, async (req, res) => {
  try {
    ...
    const [empCount,attToday,pendingLeaves,pendingBonus,payrollTotal,deptDist,recentAudit,perfAvg,payrollTrend,onLeave,topEarners,leaveByDept] = await Promise.all([
      pool.request().query(`SELECT COUNT(*) AS total,SUM(CASE WHEN Status='Active' THEN 1 ELSE 0 END) AS active FROM Employees`),
      pool.request().input('d',today).query(`SELECT COUNT(*) AS total,SUM(CASE WHEN Status IN ('Present','Half-day') THEN 1 ELSE 0 END) AS present,SUM(CASE WHEN Status='Late' THEN 1 ELSE 0 END) AS late,SUM(CASE WHEN Status='Absent' THEN 1 ELSE 0 END) AS absent,SUM(CASE WHEN Status='Half-day' THEN 1 ELSE 0 END) AS halfday FROM Attendance WHERE CAST(CheckIn AS DATE)=@d`),
      ...
      pool.request().input('y',yr).query(`SELECT Month,ISNULL(SUM(NetSalary),0) AS total FROM Payroll WHERE Year=@y GROUP BY Month ORDER BY Month`),
      pool.request().input('d',today).query(`SELECT COUNT(*) AS cnt FROM Leaves WHERE Status='Approved' AND FromDate<=@d AND ToDate>=@d`),
      pool.request().input('m',mo).input('y',yr).query(`SELECT TOP 5 e.Name,e.EmpID,d.DeptName,p.NetSalary FROM Payroll p JOIN Employees e ON p.EmpID=e.EmpID JOIN Departments d ON e.DeptID=d.DeptID WHERE p.Month=@m AND p.Year=@y ORDER BY p.NetSalary DESC`),
      pool.request().input('y',yr).query(`SELECT d.DeptName,COUNT(l.LeaveID) AS total,SUM(CASE WHEN l.Status='Approved' THEN 1 ELSE 0 END) AS approved FROM Departments d LEFT JOIN Employees e ON d.DeptID=e.DeptID LEFT JOIN Leaves l ON e.EmpID=l.EmpID AND YEAR(l.FromDate)=@y GROUP BY d.DeptName ORDER BY approved DESC`)
    ]);
    res.json({ employees:empCount.recordset[0], attendance:attToday.recordset[0], pendingLeaves:pendingLeaves.recordset[0].cnt, pendingBonus:pendingBonus.recordset[0].cnt, payrollTotal:payrollTotal.recordset[0].total, departments:deptDist.recordset, recentAudit:recentAudit.recordset, performance:perfAvg.recordset[0], payrollTrend:payrollTrend.recordset, onLeave:onLeave.recordset[0].cnt, topEarners:topEarners.recordset, leaveByDept:leaveByDept.recordset });
  } catch(err) { res.status(500).json({ error:err.message }); }
});
...
async function api(method,url,body){
  ...
  if (!r.ok) {
    ...
    throw new Error(d.error || text.slice(0,120) || r.status+' '+r.statusText);
  }
  return d;
}
...
async function loadAdminDash(){
  ...
  try{
    ...
    const trendLabels=trend.map(t=>MONTH_NAMES[t.Month-1]);
    const trendVals=trend.map(t=>Math.round(t.total/1000));
    el.innerHTML=\`
<div class="stats-grid">
  ...
  <div class="stat purple"><div class="stat-icon">\${ICONS.leave}</div><div class="stat-val">\${d.onLeave||0}</div><div class="stat-lbl">On Leave Today</div></div>
...
<div class="card" style="margin-bottom:16px">
  <div class="card-hdr" style="display:flex;justify-content:space-between;align-items:center">
    <span>Monthly Payroll (\${new Date().getFullYear()}) <span style="font-size:11px;color:var(--text3);font-weight:400">&#8377; in thousands</span></span>
    <button class="btn btn-primary btn-sm" onclick="showPage('payroll')">Generate Payroll</button>
  </div>
  <canvas id="payroll-chart" height="110"></canvas>
</div>
<div class="card">
  <div class="card-hdr">Today's Headcount <span style="font-size:11px;color:var(--text3);font-weight:400">— click to view employees</span></div>
  <div style="display:flex;gap:0;border-radius:10px;overflow:hidden;margin-top:4px">
    <div style="flex:1;text-align:center;padding:18px 10px;background:rgba(16,185,129,.1);cursor:pointer" onclick="showAttDetail('Present','Present Today','var(--green)')">
      <div style="font-size:26px;font-weight:700;color:var(--green)">\${att.present||0}</div>
      <div style="font-size:11px;color:var(--text3);margin-top:3px">PRESENT</div>
    </div>
    <div style="flex:1;text-align:center;padding:18px 10px;background:rgba(245,158,11,.1);cursor:pointer" onclick="showAttDetail('Late','Late Today','var(--yellow)')">
      <div style="font-size:26px;font-weight:700;color:var(--yellow)">\${att.late||0}</div>
      <div style="font-size:11px;color:var(--text3);margin-top:3px">LATE</div>
    </div>
    <div style="flex:1;text-align:center;padding:18px 10px;background:rgba(139,92,246,.1);cursor:pointer" onclick="showAttDetail('onleave','On Leave Today','var(--purple)')">
      <div style="font-size:26px;font-weight:700;color:var(--purple)">\${d.onLeave||0}</div>
      <div style="font-size:11px;color:var(--text3);margin-top:3px">ON LEAVE</div>
    </div>
    <div style="flex:1;text-align:center;padding:18px 10px;background:rgba(239,68,68,.1);cursor:pointer" onclick="showAttDetail('Absent','Absent Today','var(--red)')">
      <div style="font-size:26px;font-weight:700;color:var(--red)">\${att.absent||0}</div>
      <div style="font-size:11px;color:var(--text3);margin-top:3px">ABSENT</div>
    </div>
  </div>
</div>
<div class="two-col" style="margin-top:0">
  <div class="card">
    <div class="card-hdr">🏆 Top Earners This Month</div>
    \${(d.topEarners||[]).length ? (d.topEarners||[]).map((e,i)=>\`
      <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border)">
        <div style="width:24px;height:24px;border-radius:50%;background:\${['#f59e0b','#94a3b8','#b45309'][i]||'var(--border)'};display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:#fff">\${i+1}</div>
        <div class="emp-av" style="width:32px;height:32px;font-size:13px">\${(e.Name||'?')[0]}</div>
        <div style="flex:1"><div style="font-weight:500;font-size:13px">\${e.Name}</div><div style="font-size:11px;color:var(--text3)">\${e.DeptName}</div></div>
        <div style="font-weight:600;color:var(--green)">&#8377;\${(e.NetSalary||0).toLocaleString('en-IN')}</div>
      </div>\`).join('')
    : '<div style="color:var(--text3);font-size:13px;padding:12px 0">Generate payroll to see top earners</div>'}
  </div>
  <div class="card">
    <div class="card-hdr">📋 Leave Utilization by Department</div>
    \${(d.leaveByDept||[]).filter(x=>x.total>0).slice(0,6).map(x=>{
      const pct=x.total?Math.round(x.approved/x.total*100):0;
      return \`<div style="margin-bottom:10px">
        <div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:3px">
          <span style="font-weight:500">\${x.DeptName}</span>
          <span style="color:var(--text3)">\${x.approved} approved / \${x.total} total</span>
        </div>
        <div class="bar-wrap"><div class="bar" style="width:\${pct}%;background:\${pct>70?'var(--red)':pct>40?'var(--yellow)':'var(--green)'}"></div></div>
      </div>\`;
    }).join('') || '<div style="color:var(--text3);font-size:13px;padding:12px 0">No leave data yet</div>'}
  </div>
</div>\`;
    ...
    window.payrollChart=new Chart(document.getElementById('payroll-chart'),{type:'bar',data:{labels:trendLabels,datasets:[{label:'Net Payroll (₹K)',data:trendVals,backgroundColor:'rgba(59,130,246,0.75)',borderRadius:6,borderWidth:0,hoverBackgroundColor:'#3b82f6'}]},options:{plugins:{legend:{display:false},tooltip:{callbacks:{label:ctx=>'₹'+ctx.parsed.y+'K'}}},scales:{x:{ticks:{color:'#64748b'},grid:{display:false}},y:{ticks:{color:'#64748b',callback:v=>'₹'+v+'K'},grid:{color:'#f1f5f9'}}}}});
  }catch(e){el.innerHTML='<div class="alert alert-red show">'+e.message+'</div>';}
}

// ATTENDANCE DETAIL MODAL
async function showAttDetail(status, title, color) {
  try {
    const data = await GET('/api/attendance/today-detail?status=' + status);
    const isLeave = status === 'onleave';
    const rows = data.map(e => \`<tr>
      <td><div class="emp-cell"><div class="emp-av">\${(e.Name||'?')[0]}</div>\${e.Name}</div></td>
      <td style="color:var(--text3);font-size:12px">\${e.EmpID}</td>
      <td style="color:var(--text3);font-size:12px">\${e.DeptName||'--'}</td>
      <td>\${isLeave
        ? \`<span class="badge badge-purple">\${e.LeaveType||'Leave'}</span><div style="font-size:11px;color:var(--text3);margin-top:2px">\${e.Reason||''}</div>\`
        : e.CheckIn ? new Date(e.CheckIn).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}) : '--'}</td>
    </tr>\`).join('') || \`<tr><td colspan="4" style="text-align:center;color:var(--text3);padding:20px">No records found</td></tr>\`;
    openModal(title, \`<div class="tbl-wrap"><table>
      <thead><tr><th>Name</th><th>ID</th><th>Department</th><th>\${isLeave ? 'Leave Type / Reason' : 'Check-in Time'}</th></tr></thead>
      <tbody>\${rows}</tbody>
    </table></div>\`, []);
...
function showAddEmp(){
  const lastEmp=allEmps.length?allEmps.reduce((a,b)=>a.EmpID>b.EmpID?a:b,allEmps[0]):null;
  const nextNum=lastEmp?parseInt(lastEmp.EmpID.replace(/\\D/g,''))+1:1;
  const nextId='EMP'+String(nextNum).padStart(3,'0');
  openModal('Add Employee',\`<div class="form-grid">
    <div class="fg"><label>Employee ID *</label><input id="f-id" placeholder="\${nextId}" value="\${nextId}" style="text-transform:uppercase"></div>
    ...
    <div class="fg"><label>Phone</label><input id="f-phone" placeholder="9876543210" maxlength="10" oninput="this.value=this.value.replace(/\\D/g,'').slice(0,10)"></div>
...
async function showEditEmp(id){
  ...
  openModal('Edit - '+id,\`<div class="form-grid">
    ...
    <div class="fg"><label>Phone</label><input id="e-phone" value="\${emp.Phone||''}" maxlength="10" oninput="this.value=this.value.replace(/\\D/g,'').slice(0,10)"></div>
...
async function showSalComp(id,name){
  ...
  setTimeout(()=>{ const a=document.getElementById('sc-a'); if(a) a.className='alert'; }, 0);
  calcPrev();
}
function calcPrev(){
...
function openModal(title,body,btns=[]){
  ...
  if(btns.length){
    ...
    ftr.innerHTML='<button class="btn btn-ghost" onclick="closeModal()">Cancel</button>'+btns.map((b,i)=>'<button class="btn '+(b.cls||'btn-primary')+'" id="modal-btn-'+i+'">'+b.label+'</button>').join('');
    btns.forEach((b,i)=>{const el=document.getElementById('modal-btn-'+i);if(el)el.addEventListener('click',b.action);});
